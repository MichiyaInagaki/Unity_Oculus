...
public class move_sphere100_exp2 : MonoBehaviour
{
    ...
    //実験用パラメータ
    private float condition_angle_pitch = 0.0f;  //条件角度
    private int[] condition_order = new int[5]; //条件順を格納
    private int condition_num = 0;              //条件番号
    private float ajust_pitch = 0.0f;            //調整角
    private int delay_count = 0;
    private float initial_pitch = 0.0f;          //初期値
    ...
    void Update()
    {
        ...
        if (OVRInput.GetDown(OVRInput.Button.PrimaryTouchpad) && Input.GetKey(KeyCode.Escape))
        {
            if (trial_num > 10)
            {
                if (condition_num < 5)
                {
                    ...
                    condition_angle_pitch = condition_order[condition_num] * 10.0f;
                    condition_num++;
                    ajust_pitch = 0;    //調整角度リセット
        ...
        if (next_flag == true)
        {
            if (trial_num % 2 == 0)
            {
                initial_pitch = UnityEngine.Random.Range((int)(condition_angle_pitch + 15.0f), (int)(condition_angle_pitch + 45.0f));
                next_flag = false;
            }
            else
            {
                initial_pitch = UnityEngine.Random.Range(-(int)(condition_angle_pitch + 45.0f), -(int)(condition_angle_pitch + 15.0f));
                next_flag = false;
            }
        }
        ...
        if (MASK_clear == false)
        {
            if ((condition_angle_pitch + 1.0f) >= SetHalf(pitch) && SetHalf(pitch) >= (condition_angle_pitch - 1.0f))
            {
                MASK_right.SetActive(false);
                MASK_left.SetActive(false);
            }
            else if ((condition_angle_pitch + 1.0f) < SetHalf(pitch))
            {
        ...
        //補正角度の決定
        rotation_angle_pitch = initial_pitch + ajust_pitch;

        //Debug.Log
        str1 = "roll: " + roll.ToString("f1") + " pitch: " + pitch.ToString("f1") + " yaw: " + yaw.ToString("f1");
        score_object1.GetComponent<TextMesh>().text = str1;
        str2 = "condition: " + condition_angle_pitch.ToString() + " num: " + condition_num.ToString() + "-" + trial_num.ToString() + " initial: " + initial_pitch.ToString() + " ajust: " + ajust_pitch.ToString() + " rotation: " + rotation_angle_pitch.ToString();
        ...
        newAngle.x = SetOffset(rotation_angle_pitch);
        newAngle.y = -yaw;
        newAngle.z = 0;
        ...
        if (OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger))
        {
            PlaySound(trial_num);
            // ファイル書き出し
            string[] str1 = { condition_angle_pitch.ToString(), trial_num.ToString(), rotation_angle_pitch.ToString(), roll.ToString(), pitch.ToString(), yaw.ToString() };
